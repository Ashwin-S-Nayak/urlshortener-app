pipeline {
    ...
    stages {
        ...
        stage('Build Docker Images') {
            steps {
                echo '>>> STAGE 2: Building Docker images locally on Jenkins server...'
                sh '''
        ...
        stage('Run Tests') {
            steps {
                echo '>>> STAGE 3: Running 12 Jest tests against real MongoDB...'
                sh '''
                    ...
                    docker stop test-mongodb-url && docker rm test-mongodb-url || true
                ...
                sshagent(['urlshortener-public-server-key']) {
                    sh """
                        ...
                        scp -o StrictHostKeyChecking=no \
                            /home/ec2-user/urlshortener/docker-compose.yml \
                            ec2-user@${PUBLIC_SERVER}:/home/ec2-user/urlshortener/docker-compose.yml

                        scp -o StrictHostKeyChecking=no \
                            /home/ec2-user/urlshortener/nginx-site.conf \
        ...
        stage('Deploy Backend to Private Server') {
            steps {
                echo '>>> STAGE 6: Deploying backend and MongoDB to private EC2 (via SSH jump through public server)...'
                sshagent(['urlshortener-public-server-key']) {
                    sh """
                        ssh -o StrictHostKeyChecking=no ec2-user@${PUBLIC_SERVER} \
            ...
            steps {
                ...
                sshagent(['urlshortener-public-server-key']) {
                    sh """
                        ...
                        echo '--- Checking backend on private server via public server ---'
                        ssh -o StrictHostKeyChecking=no ec2-user@${PUBLIC_SERVER} \
                        "ssh -o StrictHostKeyChecking=no \
                              -i /home/ec2-user/my-server-key.pem \
                              ec2-user@${PRIVATE_SERVER} \
                        ...
                        echo '--- Checking end-to-end via Nginx proxy ---'
                        ssh -o StrictHostKeyChecking=no ec2-user@${PUBLIC_SERVER} \
    ...
    post {
        success {
            ...
            echo "Frontend: http://${env.PUBLIC_SERVER}"
            echo "API Health: http://${env.PUBLIC_SERVER}/api/health"
            echo '=============================================='
        }
        failure {
            echo '=============================================='
